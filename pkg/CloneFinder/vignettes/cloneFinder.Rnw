...
\textbf{OBSOLETE!} So, we use the posterior distribution of the
``maximum likelihood'' $\bar\varphi$ vactors as a new prior
distribution.  We sample vectors from this distribution (therefore
ensuring that we sample more vectors in a neighborhood of the most
common vectors from the first pass), and repeat the computation of
likelihoods and the idenitification for each segment of the maximum
likelihood vector.  In principle, this step could be repeated more
than once; in practice, we have not yet seen any reason to do so.
This step is implemented in the function \texttt{updatePhiVectors}.
...
@ 
\caption{Histogram of components of phi-vectors from first pass at
  modeling the simulated data.}
\label{hist.second}
\end{figure}
