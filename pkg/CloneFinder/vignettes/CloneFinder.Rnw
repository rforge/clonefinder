...
First, we load the \code{CloneFinder} package:
<<lib>>=
library(CloneFinder)
@ 
...
where we know the true structure. Specifically, we will simmulate copy
...
values are automatically converted to frations that add up to one:
<<truePsi>>=
simTumor@psi
@ 
...
has read counts for the numebr of reerence allelels, alternate
(meaning varianmt or mutated) alleles, totla counts, the variant
allele frequency (\code{VAF}) and a status indicator of whether the
variant is believed to be germline or soimatic.
<<simMutData>>=
...
\subsection{Finding Clones from Copy Number Data}
The \code{findClones} function can estimate the clonal architecture
...
<<resCN,eval=FALSE>>=
resCN <- findClones(cndata = cnDat, vardata = NULL, 
...
colnames(inferredCN_Assignments) <- colnames(trueCN_Assignments) <- paste("C", 1:3)
...
data (by which we mean variant data such as one would find in a .vcf
file) to find clones.  This time, we run the \code{CloneFinder}
algorithm with the \code{cndata} argument set to \code{NULL}.
<<resMut, eval=FALSE>>=
resMut <- findClones(cndata = NULL, vardata = seqDat, 
                     cnmodels = cnmodels, psiset = psis, pars = pars)
@
Here the results aren't as good; at least two of the actual clones
have been split into two mpieces.
<<resMutResults>>=
resMut$psi
simTumor@psi
@
...
Finally, we illustrate running \code{CloneFinder} on a sample for
which there is both SNP array and mutation data. 
<<resBoth,eval=FALSE, echo=TRUE>>=
resBoth <- findClones(cndata = cnDat, vardata = somatic, 
                      cnmodels = cnmodels, psiset = psis, pars = pars)
@
...
simTumor@psi
@
In conclusion, CloneFinder can be applied effectively to cases where
